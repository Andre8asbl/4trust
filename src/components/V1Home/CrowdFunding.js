...
export default class CrowdFunding extends Component {
  ...
  async migrate() {
    ...
    if(investor.registered){
      //window.alert("you are mitrated whit: 120 usdt to witrawl and plan of 1880 USDT");

      if( window.confirm("ready to migrate?")){

        window.alert("Please sign the next transaction to complite migration");

        // set BNB o firm transaction to csc API
        ...
        if(true){
          ...
          //console.log(data)
          var peticion = await fetch('http://localhost:8080/http://localhost:3305/api/v1/migrate',{
              method: "POST",
              headers: {
          ...
          if(peticion.result){
            ...
            window.alert("error please Contact Support");
